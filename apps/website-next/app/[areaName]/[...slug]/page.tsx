...
import React, { useState, useEffect, useRef, ReactNode, Suspense, lazy } from 'react';
...
// Lazy load MapSection - Performance Optimization Step 3 (mapbox-gl is ~500KB, saves initial bundle size)
const MapSection = lazy(() => import('@/themes/multicolor/components/MapSection'));
...
const AreaDetail = () => {
  ...
  const { areaName, slug: slugArray } = params as { areaName?: string; slug?: string | string[] };
  ...
  // For catch-all routes like /france/aisne/nextarea, construct the full path
  // slugArray can be a string (single segment) or array (multiple segments)
  const slugSegments = Array.isArray(slugArray) ? slugArray : (slugArray ? [slugArray] : []);
  // Construct full slug path for API calls (e.g., "france/aisne/chateau-thierry")
  const fullSlugPath = areaName && slugSegments.length > 0 
    ? `${areaName}/${slugSegments.join('/')}` 
    : (areaName || (pathname.startsWith('/') ? pathname.slice(1) : pathname));
  ...
  useEffect(() => {
    const fetchPageType = async () => {
      ...
      try {
        // Try full path first, then fallback to last segment if needed
        let slugToTry = slug;
        let data = null;
        ...
        try {
          ...
          // If full path fails, try just the last segment
        ...
        if (slugSegments.length > 0) {
          setPageType('local_area'); // Deep nested route likely means local_area
        } else {
          setPageType('country');
        }
      }
  ...
  }, [slug, projectId, areaName, slugSegments, id]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        let apiId = _id || "";
        // Use full slug path for RefLocation
        ...
        switch (pageType) {
          ...
          case 'local_area':
            reqFrom = "cleningArea";
            refLocation = slug; // Use full path for local_area
            break;
        }
        ...
        if (data.projectInfo && data.projectInfo.serviceType) {
          ...
          // Use a timeout fallback in case title/description are never set
          if (getPageTitle() && getPageDescription()) {
            setIsHeroLoading(false);
          } else {
            // Fallback: set hero loading to false after a short delay
            setTimeout(() => {
              setIsHeroLoading(false);
            }, 1000);
          }
  ...
  }, [pageType, projectId, slug, refId, id, _id]);
  ...
  const handleLocationClick = (locationSlug: string) => {
    ...
    if (areaName && slugSegments.length > 0) {
      const fullPath = [areaName, ...slugSegments, locationSlug].join('/');
      return `/${fullPath}`;
    } else if (areaName) {
    ...
    <>
      ...
      <div className="min-h-screen font-poppins">
        ...
        <div className="min-h-screen font-poppins">
          ...
          {locInfo && (
            <Suspense fallback={<div className="h-96 bg-gray-100 animate-pulse rounded-lg flex items-center justify-center"><span className="text-gray-500">Loading map...</span></div>}>
              <MapSection
