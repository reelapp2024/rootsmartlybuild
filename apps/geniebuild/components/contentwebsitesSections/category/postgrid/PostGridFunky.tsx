...
export const CategoryPostGridFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-catposts-title`) || {
    id: `${section.id}-cw-catposts-title`, type: 'heading',
    content: {
      text:
        c.title ||
        (c.gridMode === 'subcategories' ? 'Subcategories' : 'In this category'),
      htmlTag: 'h2',
    },
  ...
  const items: any[] = Array.isArray(c.items) && c.items.length ? c.items : [{"title":"Closet glow-up in a weekend","description":"DIY"},{"title":"Paint palettes that pop","description":"Color"},{"title":"Budget thrift finds","description":"Tips"}];
    ...
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: bg }}>
      ...
      <div className={`max-w-7xl mx-auto ${padX} ${padT} ${padB}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((item, i) => {
            const itemTitle = mergeFunkyElement(section, `${section.id}-cw-catposts-item-${i}-title`, {
              id: `${section.id}-cw-catposts-item-${i}-title`, type: 'heading',
            ...
            const itemDesc = mergeFunkyElement(section, `${section.id}-cw-catposts-item-${i}-desc`, {
              id: `${section.id}-cw-catposts-item-${i}-desc`, type: 'text',
