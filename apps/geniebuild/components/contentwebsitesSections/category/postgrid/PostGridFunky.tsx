...
export const CategoryPostGridFunky: React.FC<Props> = ({
  ...
  const { titleColor, textColor, themeMode: funkyThemeMode, themeColors: funkyThemeCfg } = funkyTextColors(tc, isLight);
  ...
  const titleEl = mergeFunkyElement(section, `${section.id}-cw-catpostgrid-title`, {
    id: `${section.id}-cw-catpostgrid-title`, type: 'heading',
    content: { text: c.title || 'Articles in this category', htmlTag: 'h2' },
  ...
  const items: any[] = Array.isArray(c.items) && c.items.length
    ? c.items
    : [
        { title: 'Starter guide', description: 'Article', link: '/blog' },
        { title: 'Deep dive', description: 'Article', link: '/blog' },
        { title: 'Checklist', description: 'Article', link: '/blog' },
      ];

  const themeColors = { ...tc, ...funkyThemeCfg, titleColor, textColor };
    ...
    <div className="relative w-full overflow-hidden" style={{ ...wrapperStyle }}>
      ...
      <div className={`max-w-7xl mx-auto ${padX} ${padT} ${padB}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((item, i) => {
            const itemTitle = mergeFunkyElement(section, `${section.id}-cw-catpostgrid-item-${i}-title`, {
              id: `${section.id}-cw-catpostgrid-item-${i}-title`, type: 'heading',
            ...
            const itemDesc = mergeFunkyElement(section, `${section.id}-cw-catpostgrid-item-${i}-desc`, {
              id: `${section.id}-cw-catpostgrid-item-${i}-desc`, type: 'text',
...
export default CategoryPostGridFunky;
