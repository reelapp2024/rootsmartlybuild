...
export const PinBoardCtaFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-pinboard-title`) || {
    id: `${section.id}-cw-pinboard-title`, type: 'heading',
    content: { text: c.title || "Steal our starter board", htmlTag: 'h2' },
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-pinboard-desc`) || {
    id: `${section.id}-cw-pinboard-desc`, type: 'text',
    content: { text: c.subtitle || c.description || "One click → curated niche pins to remix." },
  ...
  const btnEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-pinboard-btn`) || {
    id: `${section.id}-cw-pinboard-btn`, type: 'cta-button',
    content: { text: c.ctaText || c.buttonText || "Open board", href: c.ctaHref || '#' },
  ...
  return (
    <div className="relative w-full" style={{ backgroundColor: bg }}>
      ...
      <div className={`max-w-7xl mx-auto ${padX} ${padT} ${padB}`}>
        <div style={{ background: f.grape, border: `2.5px solid ${f.ink}`, borderRadius: 24, boxShadow: FUNKY.shadowLg, padding: '36px 28px', textAlign: 'center' }}>
...
export default PinBoardCtaFunky;
