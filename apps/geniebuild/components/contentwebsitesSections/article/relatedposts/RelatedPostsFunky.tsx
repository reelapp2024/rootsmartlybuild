...
export const RelatedPostsFunky: React.FC<Props> = ({
  ...
  const titleEl = mergeFunkyElement(section, `${section.id}-cw-relposts-title`, {
    id: `${section.id}-cw-relposts-title`, type: 'heading',
    content: { text: c.title || "Keep scrolling", htmlTag: 'h2' },
  ...
  const items: any[] = Array.isArray(c.items) && c.items.length ? c.items : [{"title":"Cluster map template","description":"Related"},{"title":"Pin CTA examples","description":"Related"},{"title":"Seasonal keyword list","description":"Related"}];
    ...
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: bg }}>
      ...
      <div className={`max-w-7xl mx-auto ${padX} ${padT} ${padB}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((item, i) => {
            const itemTitle = mergeFunkyElement(section, `${section.id}-cw-relposts-item-${i}-title`, {
              id: `${section.id}-cw-relposts-item-${i}-title`, type: 'heading',
            ...
            const itemDesc = mergeFunkyElement(section, `${section.id}-cw-relposts-item-${i}-desc`, {
              id: `${section.id}-cw-relposts-item-${i}-desc`, type: 'text',
...
export default RelatedPostsFunky;
