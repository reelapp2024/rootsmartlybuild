...
export const ShopTheLookFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-shop-title`) || {
    id: `${section.id}-cw-shop-title`, type: 'heading',
    content: { text: c.title || "Shop the look", htmlTag: 'h2' },
  ...
  const items: any[] = Array.isArray(c.items) && c.items.length ? c.items : [{"title":"Clip-on pegboard","description":"$24"},{"title":"Pastel washi set","description":"$12"},{"title":"Desk lamp glow","description":"$39"}];
    ...
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: bg }}>
      ...
      <div className={`max-w-7xl mx-auto ${padX} ${padT} ${padB}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((item, i) => {
            const itemTitle = mergeFunkyElement(section, `${section.id}-cw-shop-item-${i}-title`, {
              id: `${section.id}-cw-shop-item-${i}-title`, type: 'heading',
            ...
            const itemDesc = mergeFunkyElement(section, `${section.id}-cw-shop-item-${i}-desc`, {
              id: `${section.id}-cw-shop-item-${i}-desc`, type: 'text',
...
export default ShopTheLookFunky;
