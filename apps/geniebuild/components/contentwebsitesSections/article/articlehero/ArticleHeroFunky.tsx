...
/**
 * ArticleHeroFunky — funky content-site hero (GenieBuild ElementsSection compatible).
 */
export const ArticleHeroFunky: React.FC<Props> = ({
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-arthero-badge`) || {
    id: `${section.id}-cw-arthero-badge`, type: 'badge',
    content: { text: c.badgeText || "Article", icon: 'fa-sparkles', iconPosition: 'left' },
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-arthero-title`) || {
    id: `${section.id}-cw-arthero-title`, type: 'heading',
    content: { text: c.title || "How to build a niche site people actually save", htmlTag: 'h1' },
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-arthero-desc`) || {
    id: `${section.id}-cw-arthero-desc`, type: 'text',
    content: { text: c.subtitle || c.description || "By Alex Rivera · 8 min read", textSize: 'large' },
...
export default ArticleHeroFunky;
