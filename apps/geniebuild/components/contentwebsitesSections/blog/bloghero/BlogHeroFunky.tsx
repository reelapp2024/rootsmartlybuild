...
/**
 * BlogHeroFunky — funky content-site hero (GenieBuild ElementsSection compatible).
 */
export const BlogHeroFunky: React.FC<Props> = ({
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-bloghero-badge`) || {
    id: `${section.id}-cw-bloghero-badge`, type: 'badge',
    content: { text: c.badgeText || "Blog", icon: 'fa-sparkles', iconPosition: 'left' },
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-bloghero-title`) || {
    id: `${section.id}-cw-bloghero-title`, type: 'heading',
    content: { text: c.title || "The archive of awesome", htmlTag: 'h1' },
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-bloghero-desc`) || {
    id: `${section.id}-cw-bloghero-desc`, type: 'text',
    content: { text: c.subtitle || c.description || "Guides, lists & pin-ready deep dives.", textSize: 'large' },
