...
export const PostGridFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-postgrid-title`) || {
    id: `${section.id}-cw-postgrid-title`, type: 'heading',
    content: { text: c.title || "Latest posts", htmlTag: 'h2' },
  ...
  const items: any[] = Array.isArray(c.items) && c.items.length ? c.items : [{"title":"How to niche down without dying","description":"Strategy","image":"https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?w=600&q=80"},{"title":"Affiliate angles that feel honest","description":"Money","image":"https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=600&q=80"},{"title":"E-E-A-T author bios that rank","description":"SEO","image":"https://images.unsplash.com/photo-1455390582262-044cdead277a?w=600&q=80"}];
    ...
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: bg }}>
      ...
      <div className={`max-w-7xl mx-auto ${padX} ${padT} ${padB}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((item, i) => {
            const itemTitle = mergeFunkyElement(section, `${section.id}-cw-postgrid-item-${i}-title`, {
              id: `${section.id}-cw-postgrid-item-${i}-title`, type: 'heading',
            ...
            const itemDesc = mergeFunkyElement(section, `${section.id}-cw-postgrid-item-${i}-desc`, {
              id: `${section.id}-cw-postgrid-item-${i}-desc`, type: 'text',
