...
export const TermsBodyFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-terms-title`) || {
    id: `${section.id}-cw-terms-title`, type: 'heading',
    content: { text: c.title || "Terms of Use", htmlTag: 'h2' },
  ...
  const bodyEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-terms-body`) || {
    id: `${section.id}-cw-terms-body`, type: 'text',
    content: { text: c.body || c.description || c.html || "Replace with generated terms copy from your blueprint.", textSize: 'large' },
