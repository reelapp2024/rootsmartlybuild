...
/**
 * PrivacyBodyFunky — full privacy policy from SectionContent (AI queue).
 * Renders title + structured sections[{heading, bodyHtml}] like business LegalContent.
 */
export const PrivacyBodyFunky: React.FC<Props> = ({
  section,
  onTextEdit,
  buttonClass,
  onElementSelect,
  ...
  const titleEl = mergeFunkyElement(
    section,
    `${section.id}-cw-privacy-title`,
    {
      id: `${section.id}-cw-privacy-title`,
      type: 'heading',
      content: { text: c.title || 'Privacy Policy', htmlTag: 'h1' },
  ...
  const subtitleEl = mergeFunkyElement(
    section,
    `${section.id}-cw-privacy-sub`,
    {
      id: `${section.id}-cw-privacy-sub`,
      type: 'text',
      content: {
        text:
          c.subtitle ||
          'How we collect, use, and protect information on this site.' },
            ...
            {docSections.length ? (
              docSections.map((sec, i) => {
                const headingEl = mergeFunkyElement(
                  section,
                  `${section.id}-cw-privacy-h${i}`,
                  {
                    id: `${section.id}-cw-privacy-h${i}`,
                    type: 'heading',
              ...
              <p style={{ fontFamily: FUNKY.fonts.body, lineHeight: 1.7 }}>
                Privacy policy content is generating. Refresh after section generation finishes.
              </p>
            )}
          </div>
        </div>
...
export default PrivacyBodyFunky;
