...
export const PrivacyBodyFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-privacy-title`) || {
    id: `${section.id}-cw-privacy-title`, type: 'heading',
    content: { text: c.title || "Privacy Policy", htmlTag: 'h2' },
  ...
  const bodyEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-privacy-body`) || {
    id: `${section.id}-cw-privacy-body`, type: 'text',
    content: { text: c.body || c.description || c.html || "Replace with generated legal copy from your blueprint. Funky frame, readable body.", textSize: 'large' },
