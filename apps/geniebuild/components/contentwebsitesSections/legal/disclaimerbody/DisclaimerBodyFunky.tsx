...
export const DisclaimerBodyFunky: React.FC<Props> = ({
  ...
  const titleEl = mergeFunkyElement(
    section,
    `${section.id}-cw-disc-title`,
    {
      id: `${section.id}-cw-disc-title`,
      type: 'heading',
      content: { text: c.title || 'Disclaimer', htmlTag: 'h1' },
  ...
  const subtitleEl = mergeFunkyElement(
    section,
    `${section.id}-cw-disc-sub`,
    {
      id: `${section.id}-cw-disc-sub`,
      type: 'text',
      content: {
        text:
          c.subtitle ||
          'Important limits on how you should use this editorial content.' },
            ...
            {docSections.length ? (
              docSections.map((sec, i) => {
                const headingEl = mergeFunkyElement(
                  section,
                  `${section.id}-cw-disc-h${i}`,
                  {
                    id: `${section.id}-cw-disc-h${i}`,
                    type: 'heading',
              ...
              <p style={{ fontFamily: FUNKY.fonts.body, lineHeight: 1.7 }}>
                Disclaimer content is generating. Refresh after section generation finishes.
              </p>
            )}
          </div>
        </div>
