...
export const DisclaimerBodyFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-disc-title`) || {
    id: `${section.id}-cw-disc-title`, type: 'heading',
    content: { text: c.title || "Disclaimer", htmlTag: 'h2' },
  ...
  const bodyEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-disc-body`) || {
    id: `${section.id}-cw-disc-body`, type: 'text',
    content: { text: c.body || c.description || c.html || "Affiliate / editorial disclaimer — replace with generated copy.", textSize: 'large' },
...
export default DisclaimerBodyFunky;
