...
export const ContactInfoFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-coninfo-title`) || {
    id: `${section.id}-cw-coninfo-title`, type: 'heading',
    content: { text: c.title || "Reach us", htmlTag: 'h2' },
  ...
  const items: any[] = Array.isArray(c.items) && c.items.length ? c.items : [{"title":"Email","description":"hello@example.com"},{"title":"Pinterest","description":"@yourbrand"},{"title":"Instagram","description":"@yourbrand"}];
    ...
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: bg }}>
      ...
      <div className={`max-w-7xl mx-auto ${padX} ${padT} ${padB}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {items.map((item, i) => {
            const itemTitle = mergeFunkyElement(section, `${section.id}-cw-coninfo-item-${i}-title`, {
              id: `${section.id}-cw-coninfo-item-${i}-title`, type: 'heading',
            ...
            const itemDesc = mergeFunkyElement(section, `${section.id}-cw-coninfo-item-${i}-desc`, {
              id: `${section.id}-cw-coninfo-item-${i}-desc`, type: 'text',
...
export default ContactInfoFunky;
