...
/**
 * ContactHeroFunky — funky content-site hero (GenieBuild ElementsSection compatible).
 */
export const ContactHeroFunky: React.FC<Props> = ({
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-conhero-badge`) || {
    id: `${section.id}-cw-conhero-badge`, type: 'badge',
    content: { text: c.badgeText || "Contact", icon: 'fa-sparkles', iconPosition: 'left' },
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-conhero-title`) || {
    id: `${section.id}-cw-conhero-title`, type: 'heading',
    content: { text: c.title || "Slide into our inbox", htmlTag: 'h1' },
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-conhero-desc`) || {
    id: `${section.id}-cw-conhero-desc`, type: 'text',
    content: { text: c.subtitle || c.description || "Partnerships, tips, or just vibes.", textSize: 'large' },
...
export default ContactHeroFunky;
