...
export const AuthorBioFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-authbio-title`) || {
    id: `${section.id}-cw-authbio-title`, type: 'heading',
    content: { text: c.title || "Bio", htmlTag: 'h2' },
  ...
  const bodyEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-authbio-body`) || {
    id: `${section.id}-cw-authbio-body`, type: 'text',
    content: { text: c.body || c.description || c.html || "Alex builds content systems that blend Pinterest demand with honest expertise.", textSize: 'large' },
