...
/**
 * AboutHeroFunky — funky content-site hero (GenieBuild ElementsSection compatible).
 */
export const AboutHeroFunky: React.FC<Props> = ({
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-abhero-badge`) || {
    id: `${section.id}-cw-abhero-badge`, type: 'badge',
    content: { text: c.badgeText || "About us", icon: 'fa-sparkles', iconPosition: 'left' },
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-abhero-title`) || {
    id: `${section.id}-cw-abhero-title`, type: 'heading',
    content: { text: c.title || "We make niche sites feel alive", htmlTag: 'h1' },
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-abhero-desc`) || {
    id: `${section.id}-cw-abhero-desc`, type: 'text',
    content: { text: c.subtitle || c.description || "Content + pins + E-E-A-T — without the beige.", textSize: 'large' },
