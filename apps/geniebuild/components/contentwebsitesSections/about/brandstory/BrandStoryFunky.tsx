...
export const BrandStoryFunky: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-brandstory-title`) || {
    id: `${section.id}-cw-brandstory-title`, type: 'heading',
    content: { text: c.title || "Our origin story", htmlTag: 'h2' },
  ...
  const bodyEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cw-brandstory-body`) || {
    id: `${section.id}-cw-brandstory-body`, type: 'text',
    content: { text: c.body || c.description || c.html || "Started as a pin-obsessed side project. Grew into a quality-first content system for niche publishers.", textSize: 'large' },
...
export default BrandStoryFunky;
