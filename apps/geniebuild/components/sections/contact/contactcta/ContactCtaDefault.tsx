...
export const ContactCtaDefault: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = (() => {
    const id = `${section.id}-cct-title`;
    const existing = section.elements?.find(e => e.id === id);
    const c = (existing?.content || {}) as any;
    const sourceText: string = (c.text || content.title || "Prefer to Talk Right Now?").toString().replace(/<[^>]+>/g, '').trim();
  ...
  const existingPhoneSub = section.elements?.find(e => e.id === `${section.id}-cct-phone-sub`);
  const phoneSubEl: WebsiteElement = existingPhoneSub
    ...
    : {
        id: `${section.id}-cct-phone-sub`, type: 'text',
  ...
  const existingTrust = section.elements?.find(e => e.id === `${section.id}-cct-trust`);
  ...
  const trustStripEl: WebsiteElement = existingTrust
    ...
    : {
        id: `${section.id}-cct-trust`, type: 'trust-strip',
...
export default ContactCtaDefault;
