...
/**
 * AboutServiceConsistent — theme-consistent light "about this service" split
 * (image + copy). Uses the flat themeColors keys SectionRenderer provides and
 * renders the heading + body through ElementsSection so it is fully editable and
 * visually matches the other About/Service page sections.
 */
export const AboutServiceConsistent: React.FC<Props> = ({
  ...
  // sections, so colors track the active theme on theme-switch. ─────────────
  const lc = tc?.light || {};
  ...
  const title = String(pickAboutServiceTitle(content) || 'About This Service').trim();
  const bodyText = String(pickAboutServiceBody(content) || 'We deliver professional, dependable service with upfront pricing and workmanship you can rely on. Our licensed team handles every job with care, precision and respect for your home.').replace(/<[^>]+>/g, '').trim();
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-as-badge`) || {
    id: `${section.id}-as-badge`, type: 'badge',
  ...
  const bodyEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-as-body`) || {
    id: `${section.id}-as-body`, type: 'text',
...
export default AboutServiceConsistent;
