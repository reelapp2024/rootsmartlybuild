...
import { mergeSectionContent } from '../aboutservice/aboutServiceShared';
...
/**
 * ServiceHeroConsistent — theme-consistent dark service hero, matching the About
 * page hero. Uses the flat themeColors keys SectionRenderer provides and renders
 * every element through ElementsSection so it is fully editable in the builder.
 */
export const ServiceHeroConsistent: React.FC<Props> = ({
  ...
  // ── Theme tokens — flat keys from SectionRenderer (dark hero) ───────────
  ...
  // tc.backgroundColor resolves to the active theme surface (styles.backgroundColor
  // was stripped for this consistent variant), so switching themes updates the hero.
  ...
  const badgeText = String(c.serviceHeroBadge || c.badgeText || 'Our Service').trim();
  const titleText = String(c.serviceHeroTitle || c.title || 'Our Service').trim();
  const descText  = String(c.serviceHeroSubtitle || c.subtitle || c.description || 'Professional, reliable service delivered by licensed experts you can trust — every step of the way.').trim();
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-sh-badge`) || {
    id: `${section.id}-sh-badge`, type: 'badge',
    ...
    style: {
      ...
      // No backgroundColor/color — ElementsSection applies the LIVE theme badge colors.
    },
  };
  const badgeElResolved: WebsiteElement = { ...badgeEl, content: { ...(badgeEl.content || {}), text: badgeText } };

  const titleEl: WebsiteElement = (() => {
    const id = `${section.id}-sh-title`;
    const existing = section.elements?.find(e => e.id === id);
    const sourceText = titleText.replace(/<[^>]+>/g, '').trim();
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-sh-desc`) || {
    id: `${section.id}-sh-desc`, type: 'text',
  ...
  return (
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: bg }}>
      {/* Subtle accent glow — identical to About hero (no image backdrop, so both
          sub-page heroes look exactly the same on every theme). */}
      ...
      <div className={`${innerClass} relative z-10`} style={innerStyle}>
        ...
        >
          ...
          <div className="flex items-center gap-2 text-sm mt-1" style={{ color: textColor }}>
            ...
            <span>Services</span>
            <i className="fas fa-chevron-right text-[0.6rem] opacity-60" aria-hidden="true" />
            <span style={{ color: accent, fontWeight: 600 }}>{titleText}</span>
          </div>
        </motion.div>
      </div>
    </div>
...
export default ServiceHeroConsistent;
