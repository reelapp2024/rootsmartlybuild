...
/**
 * RelatedServicesDefault — a "related services" cards grid for the service page.
...
export const RelatedServicesDefault: React.FC<Props> = ({
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-rs-badge`) || {
    id: `${section.id}-rs-badge`, type: 'badge',
    content: { text: content.badgeText || 'More Services', icon: 'fa-layer-group', iconPosition: 'left', iconSize: '0.65rem' },
    style: {
  ...
  const titleEl: WebsiteElement = (() => {
    ...
    const sourceText: string = (cc.text || content.title || 'Related Services').toString().replace(/<[^>]+>/g, '').trim();
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-rs-desc`) || {
    id: `${section.id}-rs-desc`, type: 'text',
    content: { text: String(content.subtitle || (content as any).intro || 'Explore other services we offer to keep your home running smoothly.'), textSize: 'large' },
