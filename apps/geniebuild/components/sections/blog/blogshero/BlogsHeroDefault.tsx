...
/**
 * BlogsHeroDefault — clean dark sub-page hero (badge + title + subtitle +
 * breadcrumb), identical pattern to About/Contact hero so the whole site stays
 * consistent. Background + colors from the active theme; every element editable.
 */
export const BlogsHeroDefault: React.FC<Props> = ({
  ...
  const apiBadgeText = String(c.badgeText || 'Our Blog');
  const apiTitleText = String(c.blogsHeroTitle || c.title || 'Latest Articles & Insights');
  const apiDescriptionText = String(
    c.blogsHeroSubtitle || c.subtitle || c.description ||
    'Tips, guides and industry updates from our team of experts — everything you need to stay informed and make the right decisions.'
  );
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-bh-badge`) || {
    id: `${section.id}-bh-badge`, type: 'badge',
    content: { text: apiBadgeText, icon: 'fa-newspaper', iconPosition: 'left', iconSize: '0.7rem' },
    style: {
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-bh-desc`) || {
    id: `${section.id}-bh-desc`, type: 'text',
      ...
      <div className={`${innerClass} relative z-10`} style={innerStyle}>
        ...
        >
          ...
          <div className="flex items-center gap-2 text-sm mt-1" style={{ color: textColor }}>
            ...
            <span style={{ color: accent, fontWeight: 600 }}>Blog</span>
          </div>
        </motion.div>
      </div>
    </div>
...
export default BlogsHeroDefault;
