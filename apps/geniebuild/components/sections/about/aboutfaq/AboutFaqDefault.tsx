...
const DEFAULT_FAQS = [
  { title: 'How long have you been in business?',                 content: 'We have proudly served the local community for many years, building a reputation for quality workmanship, honesty and dependable service you can count on.' },
  { title: 'What areas do you serve?',                            content: 'We cover the greater metro area and surrounding neighborhoods. If you\'re unsure whether we reach your location, just give us a call and we\'ll be happy to confirm.' },
  { title: 'Is your team licensed and insured?',                 content: 'Absolutely. Every member of our team is fully licensed, background-checked and covered by comprehensive liability insurance for your complete peace of mind.' },
  { title: 'What makes your company different?',                 content: 'It\'s our people and our promise. We combine deep experience with a genuine, customer-first approach and back every job with a workmanship guarantee.' },
 ...
 */
export const AboutFaqDefault: React.FC<Props> = ({
  ...
  const apiDescriptionText = String(
    ...
    c.description ||
    c.descriptionText ||
    "Everything you might want to know about our company. Can't find what you're looking for? Our team is one call away."
  );
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-afq-badge`) || {
    id: `${section.id}-afq-badge`, type: 'badge',
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-afq-desc`) || {
    id: `${section.id}-afq-desc`, type: 'text',
    content: {
  ...
  const accordionEl: WebsiteElement = {
    id: `${section.id}-afq-accordion`, type: 'accordion',
    content: {
  ...
  const ctaTitleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-afq-cta-title`) || {
    id: `${section.id}-afq-cta-title`, type: 'heading',
    content: {
      text: faqCtaTitle,
      htmlTag: 'h3' as any,
    },
  ...
  const ctaDescEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-afq-cta-desc`) || {
    id: `${section.id}-afq-cta-desc`, type: 'text',
    content: {
...
export default AboutFaqDefault;
