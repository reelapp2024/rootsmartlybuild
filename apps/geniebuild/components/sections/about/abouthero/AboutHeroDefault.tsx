...
/**
 * AboutHeroDefault — clean light sub-page hero (badge + title + subtitle + breadcrumb).
 ...
 */
export const AboutHeroDefault: React.FC<AboutHeroProps> = ({
  ...
  const apiBadgeText = String((content as any).badgeText || 'About Us');
  const apiTitleText = String((content as any).title || 'About Our Company');
  const apiDescriptionText = String(
    (content as any).subtitle ||
    (content as any).description ||
    'Get to know the team behind the work — our story, our mission, and the values that guide everything we do for the community we proudly serve.'
  );
