...
export const CoreValuesDefault: React.FC<Props> = ({
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cv-badge`) || {
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-cv-desc`) || {
