...
export const AboutCtaDefault: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = (() => {
    const id = `${section.id}-act-title`;
    const existing = section.elements?.find(e => e.id === id);
    const c = (existing?.content || {}) as any;
    const sourceText: string = (c.text || content.title || "Ready to Work With Us?").toString().replace(/<[^>]+>/g, '').trim();
  ...
  const phoneSubEl: WebsiteElement = existingPhoneSub
    ...
    : {
        id: `${section.id}-act-phone-sub`, type: 'text',
  ...
  const trustStripEl: WebsiteElement = existingTrust
    ...
    : {
        id: `${section.id}-act-trust`, type: 'trust-strip',
...
export default AboutCtaDefault;
