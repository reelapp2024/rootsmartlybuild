...
const REASONS = [
  { icon: 'fa-medal',              title: 'Years of Proven Experience', description: 'A long track record of quality workmanship and satisfied homeowners across the community.' },
  { icon: 'fa-id-badge',           title: 'Licensed & Insured Team',     description: 'Every member of our team is fully licensed, background-checked and covered for total peace of mind.' },
  { icon: 'fa-handshake',          title: 'Honest & Transparent',        description: "We believe in straight talk and fair pricing — you'll always know exactly what to expect." },
  { icon: 'fa-heart',              title: 'Customer-First Values',       description: 'We treat every home like our own and every customer like a neighbor, because to us you are.' },
  { icon: 'fa-star',               title: 'Consistently 5-Star Rated',   description: 'Hundreds of glowing reviews from homeowners who trust us with their most important repairs.' },
  { icon: 'fa-shield-halved',      title: 'Work Backed by Guarantee',    description: 'Every job is protected by our workmanship guarantee, so you can hire us with confidence.' },
];

export const AboutWhyChooseDefault: React.FC<Props> = ({
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-awc-desc`) || {
  ...
  const getReasonEl = (i: number): WebsiteElement => {
    const id = `${section.id}-awc-card${i}`;
    const existing = section.elements?.find(e => e.id === id);
    if (existing) {
      return hideAllIcons
...
export default AboutWhyChooseDefault;
