...
export const ServicesListCtaDefault: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = (() => {
    const id = `${section.id}-slc-title`;
    const existing = section.elements?.find(e => e.id === id);
    const c = (existing?.content || {}) as any;
    const sourceText: string = (c.text || content.title || "Need One of These Services?").toString().replace(/<[^>]+>/g, '').trim();
  ...
  const phoneEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-slc-phone`) || {
    id: `${section.id}-slc-phone`, type: 'heading',
  ...
  const existingPhoneSub = section.elements?.find(e => e.id === `${section.id}-slc-phone-sub`);
  const phoneSubEl: WebsiteElement = existingPhoneSub
    ...
    : {
        id: `${section.id}-slc-phone-sub`, type: 'text',
  ...
  const existingTrust = section.elements?.find(e => e.id === `${section.id}-slc-trust`);
  ...
  const trustStripEl: WebsiteElement = existingTrust
    ...
    : {
        id: `${section.id}-slc-trust`, type: 'trust-strip',
...
export default ServicesListCtaDefault;
