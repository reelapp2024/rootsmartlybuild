...
/**
 * ServicesListHeroDefault — full copy of ServiceHeroConsistent (dark hero),
 * page-specific to the Services listing page.
 */
export const ServicesListHeroDefault: React.FC<Props> = ({
  ...
  const badgeText = String(c.serviceHeroBadge || c.badgeText || 'Our Services').trim();
  const titleText = String(c.serviceHeroTitle || c.title || 'Everything We Offer').trim();
  const descText  = String(c.serviceHeroSubtitle || c.subtitle || c.description || 'Explore our full range of professional services, delivered by licensed experts you can trust — every step of the way.').trim();
...
export default ServicesListHeroDefault;
