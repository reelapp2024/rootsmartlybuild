...
export const ServicesListProcessDefault: React.FC<Props> = ({
  ...
  const rawSteps = itemsAreMaterialized
    ...
    : STEPS.map((st, i) => ({ id: `slp-step-${i}`, ...st }));
  ...
  const handleRemoveStep = (stepId: string, idx: number) => {
    ...
    const ids = new Set([
      `${section.id}-slp-step${idx}`,
      `${section.id}-slp-step-num${idx}`,
    ]);
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-slp-badge`) || {
    id: `${section.id}-slp-badge`, type: 'badge',
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-slp-desc`) || {
    id: `${section.id}-slp-desc`, type: 'text',
  ...
  const getStepNumberEl = (i: number): WebsiteElement => {
    const id = `${section.id}-slp-step-num${i}`;
    const existing = section.elements?.find(e => e.id === id);
  ...
  const getStepEl = (i: number): WebsiteElement => {
    const id = `${section.id}-slp-step${i}`;
    const existing = section.elements?.find(e => e.id === id);
    if (existing) {
      return hideAllIcons
    ...
    <div className={`w-full ${textAlignClass}`} style={{ backgroundColor: bg }}>
      <div className={innerClass} style={innerStyle}>
        ...
        {(() => {
          ...
          const gridId = `slp-grid-${section.id.replace(/[^a-zA-Z0-9_-]/g, '_')}`;
            ...
            <div className="relative">
              ...
              <div id={gridId} className="grid gap-6 sm:gap-8">
                {rawSteps.map((step: any, i: number) => {
                  const stepId = step.id || `slp-step-${i}`;
                  return (
...
export default ServicesListProcessDefault;
