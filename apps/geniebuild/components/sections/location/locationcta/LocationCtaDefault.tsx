...
export const LocationCtaDefault: React.FC<Props> = ({
  ...
  // Padding — compact vertical rhythm
  ...
  const titleEl: WebsiteElement = (() => {
    const id = `${section.id}-lct-title`;
    const existing = section.elements?.find(e => e.id === id);
    const c = (existing?.content || {}) as any;
    const sourceText: string = (c.text || content.title || 'Need Service in Your Area Today?').toString().replace(/<[^>]+>/g, '').trim();
  ...
  const phoneEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lct-phone`) || {
    id: `${section.id}-lct-phone`, type: 'heading',
  ...
  const existingPhoneSub = section.elements?.find(e => e.id === `${section.id}-lct-phone-sub`);
  const phoneSubEl: WebsiteElement = existingPhoneSub
    ...
    : {
        id: `${section.id}-lct-phone-sub`, type: 'text',
  ...
  const existingTrust = section.elements?.find(e => e.id === `${section.id}-lct-trust`);
  ...
  const trustStripEl: WebsiteElement = existingTrust
    ...
    : {
        id: `${section.id}-lct-trust`, type: 'trust-strip',
        content: { items: trustItems } as any,
        style: trustStripStyle,
      };

  // Optional CTA button — location page uses a clear "Book Your Local Visit" action.
  const btnEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lct-btn`) || {
    id: `${section.id}-lct-btn`, type: 'button',
    content: { text: (content as any).ctaText || 'Book Your Local Visit', link: (content as any).ctaHref || '#' },
    style: {
  ...
  const btnElResolved: WebsiteElement = {
    ...btnEl,
    content: { ...(btnEl.content || {}), text: (btnEl.content as any)?.text || 'Book Your Local Visit' },
  };
    ...
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: bg }}>
      ...
      <div className={`${innerClass} relative z-10`} style={innerStyle}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          transition={{ duration: 0.45, delay: 0.35 }} className="flex justify-center mb-2">
...
export default LocationCtaDefault;
