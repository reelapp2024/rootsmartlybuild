...
/**
 * LocationGuaranteeDefault — light variant mirroring GuaranteePlumbing.
 *
 ...
 * centered CTA button.
 */
export const LocationGuaranteeDefault: React.FC<Props> = ({
  ...
  const apiTitleText = String((content as any).title || 'Our Local Satisfaction Guarantee');
  const apiDescriptionText = String(
    (content as any).subtitle ||
    (content as any).description ||
    "We stand behind every job in your area. If anything goes wrong, we'll make it right — guaranteed."
  );
  ...
  // Section bg locks to white when no user override
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lgt-badge`) || {
    id: `${section.id}-lgt-badge`, type: 'badge',
  ...
  // Description
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lgt-desc`) || {
    id: `${section.id}-lgt-desc`, type: 'text',
  ...
  const statEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lgt-stat`) || {
    id: `${section.id}-lgt-stat`, type: 'stat-card',
    content: {
      value: apiStatValue,
      text: apiStatLabel,
      icon: apiStatIcon,
  ...
  // List — all guarantee points in ONE editable element.
  ...
  const listEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lgt-list`) || {
    id: `${section.id}-lgt-list`, type: 'list',
  ...
  // CTA button
  const btnEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lgt-btn`) || {
    id: `${section.id}-lgt-btn`, type: 'button',
  ...
  return (
    <div className="relative w-full overflow-hidden" style={{ backgroundColor: bg }}>
      {/* Subtle accent halo behind the callout card */}
...
export default LocationGuaranteeDefault;
