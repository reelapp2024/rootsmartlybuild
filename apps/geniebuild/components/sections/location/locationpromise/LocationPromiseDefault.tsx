...
/**
 * LocationPromiseDefault — a short, centered promise / tagline band for the
 * location page, mirroring PromiseDefault. Follows the homepage light-section
 * pattern (theme-driven tc.light colors, white surface lock) and renders every
 * element through ElementsSection so it is fully editable in the builder.
 */
export const LocationPromiseDefault: React.FC<Props> = ({
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lpm-badge`) || {
    id: `${section.id}-lpm-badge`, type: 'badge',
  ...
  const titleEl: WebsiteElement = (() => {
    const id = `${section.id}-lpm-title`;
    const existing = section.elements?.find(e => e.id === id);
    const cc = (existing?.content || {}) as any;
    const sourceText: string = (cc.text || c.title || 'A Promise to Your Neighborhood').toString().replace(/<[^>]+>/g, '').trim();
  ...
  const lineEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lpm-line`) || {
    id: `${section.id}-lpm-line`, type: 'text',
    content: {
      text: c.subtitle || c.line || "We promise honest work, fair pricing and a job done right the first time — for every home in our community. If you're not fully satisfied, we'll make it right.",
      textSize: 'large',
    },
...
export default LocationPromiseDefault;
