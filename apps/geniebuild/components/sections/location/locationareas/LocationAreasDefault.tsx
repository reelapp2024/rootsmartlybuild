...
interface Props {
   ...
   *  and to add/remove neighborhoods. */
  onSectionUpdate?: (sectionId: string, updates: any) => void;
}

const DEFAULT_AREAS = [
  { city: 'Downtown',       state: 'TX', zip: '78701' },
  { city: 'North End',      state: 'TX', zip: '77001' },
  { city: 'Westside',       state: 'TX', zip: '75201' },
  { city: 'Riverside',      state: 'TX', zip: '78201' },
  { city: 'Old Town',       state: 'TX', zip: '76101' },
  { city: 'Oakwood',        state: 'TX', zip: '75023' },
  { city: 'Highland Park',  state: 'TX', zip: '75038' },
  { city: 'Meadowbrook',    state: 'TX', zip: '75040' },
  { city: 'Southgate',      state: 'TX', zip: '76010' },
  { city: 'Lakeview',       state: 'TX', zip: '79401' },
  { city: 'Cedar Grove',    state: 'TX', zip: '78040' },
  { city: 'Sunset Hills',   state: 'TX', zip: '78664' },
];

export const LocationAreasDefault: React.FC<Props> = ({
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lar-badge`) || {
    id: `${section.id}-lar-badge`, type: 'badge',
  ...
  const titleEl: WebsiteElement = (() => {
    const id = `${section.id}-lar-title`;
    const existing = section.elements?.find(e => e.id === id);
    const c = (existing?.content || {}) as any;
    const sourceText: string = (c.text || content.title || 'Neighborhoods We Cover').toString().replace(/<[^>]+>/g, '').trim();
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lar-desc`) || {
    id: `${section.id}-lar-desc`, type: 'text',
    content: { text: content.subtitle || 'We provide fast, reliable service across every neighborhood in the area. Not sure if we cover your street? Give us a call!', textSize: 'large' },
    style: { textAlign: 'center' as any, maxWidth: '560px', margin: '0 auto', lineHeight: '1.65' },
  };

  const btnEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lar-btn`) || {
    id: `${section.id}-lar-btn`, type: 'button',
  ...
  const phoneEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lar-phone`) || {
    id: `${section.id}-lar-phone`, type: 'button',
  ...
  const ctaNoteEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lar-note`) || {
    id: `${section.id}-lar-note`, type: 'text',
    content: { text: "Don't see your neighborhood? We may still cover your street!", textSize: 'small' },
    style: { fontSize: '0.875rem' },
  };

  // ── Neighborhoods — each is its own editable `badge` element.
  ...
  const cityItems = itemsAreMaterialized
    ...
    : DEFAULT_AREAS.map((a, i) => ({ id: `lar-city-${i}`, city: a.city, link: '', locationId: '' }));

  const materializeIfNeeded = (): any[] => {
    if (itemsAreMaterialized) return cityItems;
    if (!onSectionUpdate) return cityItems;
    const seeded = DEFAULT_AREAS.map((a, i) => ({ id: `lar-city-${i}`, title: a.city }));
  ...
  const handleAddCity = () => {
    ...
    materializeIfNeeded();
    const newItem = { id: `lar-city-${Date.now()}`, title: 'New Area' };
    const seeded = (content.items && content.items.length > 0
      ? (content.items as any[])
      : DEFAULT_AREAS.map((a, i) => ({ id: `lar-city-${i}`, title: a.city })));
  ...
  const handleRemoveCity = (cityId: string, idx: number) => {
    ...
    const elementIdToRemove = `${section.id}-lar-city${idx}`;
  ...
  return (
    <div className={`relative w-full overflow-hidden ${textAlignClass}`} style={{ backgroundColor: bg }}>
      ...
      <div className={`${innerClass} relative z-10`} style={innerStyle}>
        ...
        {/* Neighborhood pill cloud — flowing flex-wrap of editable badge elements. */}
        ...
        >
          {cityItems.map((city: any, i: number) => (
            ...
            >
              ...
              {isSelected && !readOnly && onSectionUpdate && cityItems.length > 1 && (
                <button
                  onClick={(e) => { e.stopPropagation(); handleRemoveCity(city.id || `lar-city-${i}`, i); }}
          ...
          {/* Add-neighborhood pill — only when section is selected */}
...
export default LocationAreasDefault;
