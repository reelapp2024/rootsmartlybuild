...
const STEPS = [
  { icon: 'fa-phone',            title: 'Call or Book Online', description: 'Reach us 24/7 by phone or online. We confirm your local appointment instantly.' },
  { icon: 'fa-truck-fast',       title: 'We Come to You',      description: 'A licensed local technician arrives on time, right in your neighborhood, ready to work.' },
  { icon: 'fa-wrench',           title: 'Expert Repair',       description: 'We diagnose and fix the problem using premium materials — done right the first time.' },
...
export const LocationProcessDefault: React.FC<Props> = ({
  ...
  const rawSteps = itemsAreMaterialized
    ...
    : STEPS.map((st, i) => ({ id: `lpr-step-${i}`, ...st }));
  ...
  const handleRemoveStep = (stepId: string, idx: number) => {
    ...
    const ids = new Set([
      `${section.id}-lpr-step${idx}`,
      `${section.id}-lpr-step-num${idx}`,
    ]);
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lpr-badge`) || {
    id: `${section.id}-lpr-badge`, type: 'badge',
  ...
  const titleEl: WebsiteElement = (() => {
    const id = `${section.id}-lpr-title`;
    const existing = section.elements?.find(e => e.id === id);
    const c = (existing?.content || {}) as any;
    const sourceText: string = (c.text || content.title || 'How We Work in Your Area').toString().replace(/<[^>]+>/g, '').trim();
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-lpr-desc`) || {
    id: `${section.id}-lpr-desc`, type: 'text',
    content: { text: content.subtitle || 'Four simple steps from your call to a fully fixed home — fast, clean and professional, right in your neighborhood.', textSize: 'large' },
    style: { textAlign: 'center' as any, maxWidth: '560px', margin: '0 auto', lineHeight: '1.65' },
  };

  // Step number — editable text element.
  const getStepNumberEl = (i: number): WebsiteElement => {
    const id = `${section.id}-lpr-step-num${i}`;
    const existing = section.elements?.find(e => e.id === id);
  ...
  const getStepEl = (i: number): WebsiteElement => {
    const id = `${section.id}-lpr-step${i}`;
    const existing = section.elements?.find(e => e.id === id);
    if (existing) {
      return hideAllIcons
            ...
            <div className="relative">
              <style>{`
                ...
                @media (min-width: 640px) {
                  ...
                  #${gridId} > .lpr-step-cell:nth-child(2n)::after,
                  #${gridId} > .lpr-step-cell:last-of-type::after { content: none; }
                }
                @media (min-width: 1024px) {
                  ...
                  #${gridId} > .lpr-step-cell:nth-child(2n)::after { content: ''; }
                  #${gridId} > .lpr-step-cell:nth-child(${desktopCols}n)::after,
                  #${gridId} > .lpr-step-cell:last-of-type::after { content: none; }
                }
              `}</style>
              <div id={gridId} className="grid gap-6 sm:gap-8">
                {rawSteps.map((step: any, i: number) => {
                  const stepId = step.id || `lpr-step-${i}`;
                  return (
                    <motion.div key={stepId}
                      initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }}
                      viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}
                      className="lpr-step-cell relative text-center group/step"
                    >
...
export default LocationProcessDefault;
