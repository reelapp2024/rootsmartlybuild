...
/**
 * SubLocationsDefault — grid of sub-locations (states / cities / areas /
 * neighborhoods) as clickable cards. Light section following the homepage
 * tc.light pattern; header via ElementsSection, cards theme-styled.
 */
export const SubLocationsDefault: React.FC<Props> = ({
  ...
  const rawItems = (content.items && content.items.length > 0)
    ? content.items.map((it: any, i: number) => ({
        name: it.name || it.title || it.label || DEFAULT_LOCATIONS[i % DEFAULT_LOCATIONS.length].name,
        meta: it.meta || it.subtitle || it.description || DEFAULT_LOCATIONS[i % DEFAULT_LOCATIONS.length].meta,
      }))
    : DEFAULT_LOCATIONS;
  ...
  const titleEl: WebsiteElement = (() => {
    ...
    const sourceText = (cc.text || content.title || 'Explore Nearby Locations').toString().replace(/<[^>]+>/g, '').trim();
