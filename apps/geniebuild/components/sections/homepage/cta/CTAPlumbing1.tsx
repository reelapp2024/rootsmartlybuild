...
export const CTAPlumbing1: React.FC<Props> = ({
  ...
  const trustStripEl: WebsiteElement = existingTrust
    ...
    : {
        id: `${section.id}-c1-trust`, type: 'trust-strip',
        content: { items: trustItems } as any,
        style: trustStripStyle,
      };

  // Layered ambience built from theme tokens — no hex hardcoded.
  // Two off-axis radial glows (accent-tinted) + a faint conic sweep + a subtle
  // grid pattern. All stack above the section bg so the user can still change
  // bg color/image from the sidebar and the layers tint to it.
