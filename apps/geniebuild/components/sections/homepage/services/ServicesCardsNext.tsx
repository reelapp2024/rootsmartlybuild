...
export const ServicesCardsNext: React.FC<Props> = ({
  ...
  return (
    <div className={`w-full ${uid}`} style={{ backgroundColor: bg }}>
      {/* Card hover: accent border + cursor-follow glow + subtle lift */}
      <style>{`
        .${uid} .sc-card { position:relative; transition:border-color .3s, transform .3s, box-shadow .3s; }
        .${uid} .sc-card::after { content:''; position:absolute; inset:0; border-radius:1rem; opacity:0; transition:opacity .3s; pointer-events:none; background:radial-gradient(340px circle at var(--sx,50%) var(--sy,50%), ${accent}12, transparent 70%); }
        .${uid} .sc-card:hover { border-color:${accent}66 !important; transform:translateY(-4px); box-shadow:0 18px 40px -22px ${accent}55; }
        .${uid} .sc-card:hover::after { opacity:1; }
        .${uid} .sc-card:hover .sc-icon { border-color:${accent}66 !important; }
      `}</style>
      {svcModal.open && (
