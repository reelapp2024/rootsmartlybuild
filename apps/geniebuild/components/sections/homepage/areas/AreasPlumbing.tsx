...
export const AreasPlumbing: React.FC<Props> = ({
  ...
  const materializeIfNeeded = (): any[] => {
    ...
    const seeded = DEFAULT_AREAS.map((a, i) => ({ id: `ap-city-${i}`, title: a.city }));
  ...
  const handleAddCity = () => {
    ...
    const newItem = { id: `ap-city-${Date.now()}`, title: 'New City' };
    const seeded = (content.items && content.items.length > 0
      ? (content.items as any[])
      : DEFAULT_AREAS.map((a, i) => ({ id: `ap-city-${i}`, title: a.city })));
  ...
  const handleRemoveCity = (cityId: string, idx: number) => {
    ...
    const elementIdToRemove = `${section.id}-ap-city${idx}`;
  ...
  const getCityEl = (i: number, cityName: string, cityLink?: string): WebsiteElement => {
    ...
    if (existing) {
      return {
        ...existing,
        content: { ...existingContent, ...mergedContent },
      };
    }
          ...
          {cityItems.map((city: any, i: number) => (
            ...
            >
              ...
              {isSelected && !readOnly && onSectionUpdate && cityItems.length > 1 && (
                <button
                  onClick={(e) => { e.stopPropagation(); handleRemoveCity(city.id || `ap-city-${i}`, i); }}
