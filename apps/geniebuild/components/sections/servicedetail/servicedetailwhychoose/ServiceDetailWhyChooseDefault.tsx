...
const REASONS = [
  { icon: 'fa-medal',              title: 'Specialists in This Service',   description: 'This is exactly what we do every day, so you get focused expertise instead of a general fix.' },
  { icon: 'fa-id-badge',           title: 'Licensed & Insured Team',       description: 'Every technician handling this service is fully licensed, background-checked and insured.' },
  { icon: 'fa-screwdriver-wrench', title: 'Right Tools for the Job',       description: 'We arrive fully equipped with the proper tools and parts to complete this service correctly.' },
  { icon: 'fa-clock',              title: 'On-Time, Every Time',            description: 'We respect your schedule and show up when we say we will, ready to get straight to work.' },
  { icon: 'fa-star',               title: 'Proven Results',                description: 'Countless completed jobs and glowing reviews from customers who chose us for this exact service.' },
  { icon: 'fa-shield-halved',      title: 'Backed by Our Guarantee',       description: 'This service is protected by our workmanship guarantee, so you can book us with confidence.' },
];

export const ServiceDetailWhyChooseDefault: React.FC<Props> = ({
  ...
  const titleEl: WebsiteElement = (() => {
    ...
    const sourceText: string = (c.text || content.title || 'Why Choose Us for This Service').toString().replace(/<[^>]+>/g, '').trim();
...
export default ServiceDetailWhyChooseDefault;
