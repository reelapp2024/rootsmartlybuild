...
/**
 * ServiceDetailHeroDefault — full copy of ServiceHeroConsistent (dark hero),
 * page-specific to the Service detail page.
 */
export const ServiceDetailHeroDefault: React.FC<Props> = ({
  ...
  const titleText = String(c.serviceHeroTitle || c.title || 'Professional Service You Can Trust').trim();
  const descText  = String(c.serviceHeroSubtitle || c.subtitle || c.description || 'Trusted, professional service delivered by licensed experts who treat your home with care, every step of the way.').trim();
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-sdh-badge`) || {
    id: `${section.id}-sdh-badge`, type: 'badge',
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-sdh-desc`) || {
    id: `${section.id}-sdh-desc`, type: 'text',
...
export default ServiceDetailHeroDefault;
