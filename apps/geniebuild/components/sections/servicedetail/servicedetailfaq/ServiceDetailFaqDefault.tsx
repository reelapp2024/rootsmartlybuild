...
const DEFAULT_FAQS = [
  { title: 'How long does this service take?',                     content: 'Most jobs are completed within a single visit, but the exact time depends on the scope of the work. We\'ll give you a clear estimate up front so you know what to expect.' },
  { title: 'Do I need to prepare anything beforehand?',            content: 'In most cases, no special preparation is required. If there\'s anything specific we\'ll need access to, we\'ll let you know when we confirm your appointment.' },
  { title: 'Is this service guaranteed?',                          content: 'Yes. We stand behind our work with a satisfaction guarantee, and we\'ll make it right if anything isn\'t up to your expectations.' },
  { title: 'How much does this service cost?',                     content: 'Pricing depends on the size and details of the job. Reach out for a clear, no-obligation quote before any work begins so there are no surprises.' },
  { title: 'Do you offer emergency availability for this service?', content: 'Yes. For urgent situations we offer emergency availability. Call us and we\'ll let you know how quickly we can get someone out to you.' },
  { title: 'How soon can you schedule this service?',              content: 'We do our best to accommodate same-day and next-day requests. Contact us with your preferred timing and we\'ll find a slot that works for you.' },
];

/**
 * ServiceDetailFaqDefault — full copy of FAQPlumbing, page-specific to the Service Detail page.
 */
export const ServiceDetailFaqDefault: React.FC<Props> = ({
  ...
  const apiTitleText = String(c.title || c.heading || 'Service FAQs');
  ...
  const badgeEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-sdf-badge`) || {
    id: `${section.id}-sdf-badge`, type: 'badge',
  ...
  const descEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-sdf-desc`) || {
    id: `${section.id}-sdf-desc`, type: 'text',
    content: {
    ...
    : readOnly
      ...
      : (() => {
          const accordionEl = section.elements?.find(
            (e) => e.type === 'accordion' && String(e.id || '').includes('-sdf-accordion')
          );
  ...
  const savedAccordion = section.elements?.find(e => e.id === `${section.id}-sdf-accordion`);
  ...
  const accordionEl: WebsiteElement = {
    id: `${section.id}-sdf-accordion`, type: 'accordion',
    content: {
  ...
  const ctaTitleEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-sdf-cta-title`) || {
    id: `${section.id}-sdf-cta-title`, type: 'heading',
    content: {
      text: faqCtaTitle,
      htmlTag: 'h3' as any,
    },
  ...
  const ctaDescEl: WebsiteElement = section.elements?.find(e => e.id === `${section.id}-sdf-cta-desc`) || {
    id: `${section.id}-sdf-cta-desc`, type: 'text',
    content: {
...
export default ServiceDetailFaqDefault;
