...
interface SelectElementSettingsProps {
  element: Element;
  sectionId: string;
  rowId: string;
  columnId: string;
...
export default function SelectElementSettings({
  element,
  sectionId,
  rowId,
  columnId,
  activeTab,
  updateElement,
}: SelectElementSettingsProps) {
  // Get options as string (one per line)
  const getOptionsString = () => {
    if (!element.data?.options) return '';
    if (typeof element.data.options === 'string') return element.data.options;
    if (Array.isArray(element.data.options)) return element.data.options.join('\n');
    return '';
  };
          ...
          <div className="mb-3">
            ...
            <textarea
              ...
              onChange={(e) => {
                ...
                updateElement(sectionId, rowId, columnId, element.id, { data: { ...element.data, options: options } });
              }}
