...
export const uniqueId = "cta_d";

// Variant D: Split layout with gradient background
const DEFAULT_ELEMENTS: DefaultElement[] = [
  {
    ...
    defaultStyle: {
      ...
      background: 'linear-gradient(135deg, var(--color-gradient-from, #0E1214), var(--color-gradient-to, #1F2937))', // Gradient background
...
export default function CTAD(props: CTASectionProps) {
    ...
    if (ctasToDisplay.length > 0 && existingCTAs.length === 0) {
      ctasToDisplay.forEach((cta, index) => {
        ...
        __studio.updateCustomElementStyle?.(ctaContainerId, {
          ...
          gap: '40px',
          padding: '60px',
          background: 'linear-gradient(135deg, var(--color-gradient-from, #0E1214), var(--color-gradient-to, #1F2937))', // Theme gradient
          borderRadius: '20px',
          width: '100%',
        });
        ...
        if (customElements.length === 0) {
          if (builderMode) {
            return renderEmptyState({
              ...
              emptyMessage: '[CTAD] No elements found - initializing default structure...'
            });
          } else {
            if (loading) {
                  ...
                  {ctasToDisplay.map((cta, index) => (
                    ...
                    >
                      <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                        ...
                        <p style={{ fontSize: '1.125rem', color: '#ffffff', lineHeight: 1.6, margin: 0, marginBottom: '24px', opacity: 0.9 }}>
